<?php
/*
Plugin Name: WP-Polls (with CubePoints)
Plugin URI: http://cubepoints.com/forums/topic/cp-polls/
Description: [MODIFIED TO WORK WITH THE CUBEPOINTS PLUGIN] [Based on WP-Polls Version 2.5] Adds an AJAX poll system to your WordPress blog. You can easily include a poll into your WordPress's blog post/page. WP-Polls is extremely customizable via templates and css styles and there are tons of options for you to choose to ensure that WP-Polls runs the way you wanted. It now supports multiple selection of answers.
Version: 1.0
Author: Lester 'GaMerZ' Chan (Modified by the CubePoints Team)
...
function poll_menu() {
	...
	if (function_exists('add_submenu_page')) {
		...
		add_submenu_page('wp-polls/polls-manager.php', __('CubePoints', 'wp-polls'), __('CubePoints', 'wp-polls'), 'manage_polls', 'wp-polls/polls-cubepoints.php');
...
function poll_scripts_admin($hook_suffix) {
	global $text_direction;
	$poll_admin_pages = array('wp-polls/polls-manager.php', 'wp-polls/polls-add.php', 'wp-polls/polls-options.php', 'wp-polls/polls-cubepoints.php', 'wp-polls/polls-templates.php', 'wp-polls/polls-uninstall.php');
			...
			if($check_voted == 0) {
				...
				if(function_exists('cp_alterPoints')){
					...
					if( 0 != $current_user->ID ){
						cp_alterPoints($current_user->ID, get_option('poll_cubepoints'));
						cp_log('Poll',$current_user->ID,get_option('poll_cubepoints'),'Poll #'.$poll_id);
					}
				}
...
function create_poll_table() {
	...
	// Options for CubePoints
	add_option('poll_cubepoints', 2);
